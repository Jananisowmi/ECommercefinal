...
public class UserOrderServiceTest {
	...
	public void testUserOrderNegative() throws StoreNotFoundException, UserNotPresentException {
		UserOrderDto userOrderDto = new UserOrderDto();
		userOrderDto.setUserId(1);
		userOrderDto.setStoreName("dmart");
		
		when(userRepository.findUserByUserId(userOrderDto.getUserId())).thenReturn(new User());
		when(storeRepository.findByStoreName(userOrderDto.getStoreName())).thenReturn(null);
		userOrderServiceImpl.userOrder(userOrderDto);
	...
	public void testListUserOrder() throws UserNotPresentException, UserOrderListException {
	   ...
	   List<UserOrder>	listUserOrderexpected = userOrderServiceImpl.listUserOrder(1);
	   assertEquals(listUserOrderexpected, listuserOrder);
	...
	public void testListUserOrderNegative() throws UserNotPresentException, UserOrderListException {
		...
		Mockito.when(userOrderRepository.findByUserId(1)).thenReturn(listuserOrder);
		 List<UserOrder> listUserOrderexpected = userOrderServiceImpl.listUserOrder(null);
		  assertEquals(listUserOrderexpected, listuserOrder);
